...
# This Terraform configuration deploys the Django backend to AWS
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "elearn/backend/terraform.tfstate"
    # region = "us-east-1"
  }
}
  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "curl -f http://localhost:8000/api/courses/ || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 60
